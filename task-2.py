...
task_2_data_dir = './TASK-2-DATA-NEW'
curr_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
task_2_model_save_path = 'output-2-new-mean-(1->2)/task-2-'+model_name.replace("/", "-")+'-'+ curr_time
...
evaluator = CustomLabelAccuracyEvaluator(dev_dataloader, softmax_model=train_loss, name='task2-dev', device=device_name)
...
softmax_loss_path = os.path.join(task_2_model_save_path, "softmax_loss.pth")
...
test_evaluator = CustomLabelAccuracyEvaluator(test_dataloader, name='task2-test', softmax_model=test_loss, device=device_name)
