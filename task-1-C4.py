...
def read_data_from_path(path, max_label):
    ...
    for d in dirs:
        ...
        print('max label:{} label:{}'.format(max_label, label))
        if max_label - 1 < label:
            continue
        print('label-before:{}'.format(label))
        label = max_label - 1 - label
        print('label-after:{}'.format(label))
...
evaluator = CustomLabelAccuracyEvaluator(dev_dataloader, softmax_model=train_loss, name='task1-dev-C4', device=device_name)
...
test_evaluator = CustomLabelAccuracyEvaluator(test_dataloader, name='task1-test-C4', softmax_model=train_loss, device=device_name)
